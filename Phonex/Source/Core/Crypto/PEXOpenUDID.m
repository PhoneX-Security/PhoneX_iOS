...
//  Copyright 2011, 2012 PEXOpenUDID.org
//
//  Initiators/root branches
//      iOS code: https://github.com/ylechelle/PEXOpenUDID
//      Android code: https://github.com/vieux/PEXOpenUDID
//
//  Contributors:
//      https://github.com/ylechelle/PEXOpenUDID/contributors
//
...
static NSString * const kOpenUDIDKey = @"PEXOpenUDID";
...
static NSString * const kOpenUDIDDomain = @"org.PEXOpenUDID";
static NSString * const kOpenUDIDSlotPBPrefix = @"org.PEXOpenUDID.slot.";
static int const kOpenUDIDRedundancySlots = 100;

@interface PEXOpenUDID (Private)
...
// Private method to create and return a new PEXOpenUDID
// Theoretically, this function is called once ever per application when calling [PEXOpenUDID value] for the first time.
// After that, the caching/pasteboard/redundancy mechanism inside [PEXOpenUDID value] returns a persistent and cross application PEXOpenUDID
//
+ (NSString*) _generateFreshOpenUDID {
...
//    }
    ...
    // compile time. Accredited developers integrating PEXOpenUDID are expected to review the iOS 6 release notes and
    ...
    //
    // Essentially, this means that PEXOpenUDID will keep on behaving the same way as before for existing users or
    // new users in iOS 5 and before. For new users on iOS 6 and after, the new official public APIs will take over.
    // PEXOpenUDID will therefore be obsoleted when iOS reaches significant adoption, anticipated around mid-2013.
...
// Main public method that returns the PEXOpenUDID
// This method will generate and store the PEXOpenUDID if it doesn't exist, typically the first time it is called
...
// Otherwise, it will register the current app and return the PEXOpenUDID
//
+ (NSString*) value {
    return [PEXOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if (kOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                     ...
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"PEXOpenUDID in cache from first call",@"description", nil]];
    ...
    // Do we have a local copy of the PEXOpenUDID dictionary?
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [PEXOpenUDID _getDictFromPasteboard:slotPB];
    ...
    // sort the Frequency dict with highest occurence count of the same PEXOpenUDID (redundancy, failsafe)
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            // this is the case where this app instance is likely to be the first one to use PEXOpenUDID on this device
            // we create the PEXOpenUDID, legacy or semi-random (i.e. most certainly unique)
            //
            openUDID = [PEXOpenUDID _generateFreshOpenUDID];
        } else {
            // or we leverage the PEXOpenUDID shared by other apps that have already gone through the process
            // 
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [PEXOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
    ...
    // If the UIPasteboard external representation marks this app as opted-out, then to respect privacy, we return the ZERO PEXOpenUDID, a sequence of 40 zeros...
    ...
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:kOpenUDIDDomain
                                                 ...
                                                 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Application with unique id %@ is opted-out from PEXOpenUDID as of %@",appUID,optedOutDate],@"description", nil]];
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                     ...
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Found a discrepancy between stored PEXOpenUDID (reliable) and redundant copies; one of the apps on the device is most likely corrupting the PEXOpenUDID protocol",@"description", nil]];
        else
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                         code:kOpenUDIDErrorNone
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"PEXOpenUDID succesfully retrieved",@"description", nil]];
    }
