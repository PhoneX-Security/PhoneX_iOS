...
static const void *PEXRetainCallBack(CFAllocatorRef allocator, const void *value) { return CFRetain(value); }
static void PEXReleaseCallBack(CFAllocatorRef allocator, const void *value)       { CFRelease(value); }
static void PEXRunLoopPerformCallBack(void *info);

#pragma mark - PEXRunLoopInputStream()
@interface PEXRunLoopInputStream() {
...
@implementation PEXRunLoopInputStream {
...
-(void) setupRunLoopingIfNeeded {
    if (_runloopingInitialized){
        return;
    }

    DDLogWarn(@"Delayed runlooping initialization");
...
-(void) setupCFRunlooping {
    ...
    CFRunLoopSourceContext runLoopSourceContext = {
            0, (__bridge void *)(self), NULL, NULL, NULL, NULL, NULL, NULL, NULL, PEXRunLoopPerformCallBack
    };
    ...
    CFDictionaryValueCallBacks runLoopsModesValueCallBacks = {
            0, PEXRetainCallBack, PEXReleaseCallBack, NULL, CFEqual
    };
...
- (void)addMode:(CFStringRef)mode forRunLoop:(CFRunLoopRef)runLoop {
    ...
    if (!CFDictionaryContainsKey(_runLoopsModes, runLoop)) {
        CFSetCallBacks modesSetCallBacks = {
                0, PEXRetainCallBack, PEXReleaseCallBack, NULL, CFEqual, CFHash
        };
...
- (void)notifyClientCallback:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode {
    ...
    if (_clientCallback && (eventCode & _clientFlags) && _shouldNotifyCoreFoundationAboutStatusChange) {
        _clientCallback((__bridge CFReadStreamRef)self, (CFStreamEventType)eventCode, _clientContext.info);
    }
}
...
void PEXRunLoopPerformCallBack(void *info) {
    PEXRunLoopInputStream *stream = (__bridge PEXRunLoopInputStream *)info;
