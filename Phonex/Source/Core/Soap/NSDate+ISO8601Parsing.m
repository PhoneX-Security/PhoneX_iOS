        ...
        case 2:
          switch(num_leading_hyphens) {
            case 0:
              if(*ch == '-') {
                ...
                if(*++ch == 'W')
                  ...
                  DDLogVerbose(@"(%@) parsing month; segment is %u and ch is %s", str, segment, ch);
                  switch(num_digits) {
                    case 4: //YY-MMDD
                      day = segment % 100U;
                      month_or_week = segment / 100U;
            ...
            case 1:; //-YY; -YY-MM (implicit century)
              DDLogVerbose(@"(%@) found %u digits and one hyphen, so this is either -YY or -YY-MM; segment (year) is %u", str, num_digits, segment);
              ...
              if(*ch == '-') {
                ++ch;
                month_or_week = read_segment_2digits(ch, &ch);
                DDLogVerbose(@"(%@) month is %u", str, (unsigned int) month_or_week);
              } else {
                month_or_week = 1U;
              }
