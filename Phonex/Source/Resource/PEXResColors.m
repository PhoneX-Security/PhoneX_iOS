...
// TODO projection from color values to purpose colors according to
// Gui manual (create also)

@implementation PEXResColors

+ (UIColor *) color:(NSString * const) key
{
...
+ (void) initColors
{
    const NSDictionary * const hexColors =
        [NSDictionary dictionaryWithContentsOfFile:[[PEXTheme getCurrentThemeBundle]
                                                pathForResource:@"colors"
                                                ofType:@"plist"]];

    const NSDictionary * const hexColorsIndependent =
    ...
    for (const id key in hexColorsIndependent)
    {
        [colors setObject: [self colorFromHexString:[hexColorsIndependent objectForKey:key]]
                   forKey: key];
    }
...
+ (UIColor *) colorFromHexString: (NSString * const) hexString
{
    NSString * const colorString =
...
+ (CGFloat) colorComponentFrom: (NSString * const) string
