//
//  PEXPjZrtpTransport.c
//  Phonex
//
//  Created by Dusan Klinec on 09.12.14.
//  Copyright (c) 2014 PhoneX. All rights reserved.
//
#include "PEXPjZrtpTransport.h"
#include "PEXPjZrtpTransportProtected.h"

/* Code only if ZRTP support */
#include <pjmedia/endpoint.h>
#include <pjlib.h>
#include <pjlib-util.h>
#include "PEXPjZsrtpCWrapper.h"
#include <pjnath/ice_strans.h>
#include <inttypes.h>
#import "pjmedia/transport_ice.h"
#include "PEXPjZsrtpCWrapper.h"
#import "ZrtpCWrapper.h"
#include "PEXPjZrtpTimer.h"
...
{
    &pj_zrtp_transport_get_info,
    &pj_zrtp_transport_attach,
    &pj_zrtp_transport_detach,
    &pj_zrtp_transport_send_rtp,
    &pj_zrtp_transport_send_rtcp,
    &pj_zrtp_transport_send_rtcp2,
    &pj_zrtp_transport_media_create,
    &pj_zrtp_transport_encode_sdp,
    &pj_zrtp_transport_media_start,
    &pj_zrtp_transport_media_stop,
    &pj_zrtp_transport_simulate_lost,
    &pj_zrtp_transport_destroy
};
...
PJ_DECL(pj_status_t) pjmedia_transport_zrtp_initialize(pjmedia_transport *tp, const char *zidFilename, pj_bool_t autoEnable)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
    PJ_ASSERT_RETURN(tp, PJ_EINVAL);
    DBGOUTI("ZRTP initialize");
    zrtp_initializeZrtpEngine(zrtp->zrtpCtx, &c_callbacks, zrtp->clientIdString, zidFilename, zrtp, zrtp->mitmMode);
...
#ifndef DYNAMIC_TIMER
    zrtp_timer_add_entry(&zrtp->timeoutEntry, &timeout);
#else
...
#ifndef DYNAMIC_TIMER
    zrtp_timer_cancel_entry(&zrtp->timeoutEntry);
#else
    if(zrtp->timer_heap != NULL){
 ...
 */
static pj_status_t pj_zrtp_transport_get_info(pjmedia_transport *tp, pjmedia_transport_info *info)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
 ...
 */
static pj_status_t pj_zrtp_transport_attach(pjmedia_transport *tp,
                                    void *user_data,
 ...
 */
static void pj_zrtp_transport_detach(pjmedia_transport *tp, void *strm)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
 ...
 */
static pj_status_t pj_zrtp_transport_send_rtp(pjmedia_transport *tp, const void *pkt, pj_size_t size)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
    ...
    //PJ_LOG(4, (THIS_FILE, "pj_zrtp_transport_send_rtp, started: %d enabled: %d", zrtp->started, zrtp->enableZrtp));
    if (!zrtp->started && zrtp->enableZrtp)
    {
        if (zrtp->localSSRC == 0)
    ...
    if (pkt!=NULL && size > 8 && (rand() % 50) == 1){
    	zrtp_addEntropy((uint8_t *) pkt, size < 32 ? size : 32);
    }
 ...
 */
static pj_status_t pj_zrtp_transport_send_rtcp(pjmedia_transport *tp, const void *pkt, pj_size_t size)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
    pj_status_t rc = PJ_SUCCESS;
    int32_t newLen = 0;
 ...
 */
static pj_status_t pj_zrtp_transport_send_rtcp2(pjmedia_transport *tp,
 ...
 * a new call.
 */
static pj_status_t pj_zrtp_transport_media_create(pjmedia_transport *tp,
        pj_pool_t *sdp_pool,
        unsigned options,
 ...
 */
static pj_status_t pj_zrtp_transport_encode_sdp(pjmedia_transport *tp,
                                        pj_pool_t *sdp_pool,
 ...
 */
static pj_status_t pj_zrtp_transport_media_start(pjmedia_transport *tp,
        pj_pool_t *pool,
        ...
        unsigned media_index)
{
    ...
    if (rem_sdp){
    	zrtp->zrtpHashCheckPostponed = PJ_FALSE;
    	PJ_LOG(4,(THIS_FILE, "pj_zrtp_transport_media_start, going to examine zrtp-hash"));
 ...
 */
static pj_status_t pj_zrtp_transport_media_stop(pjmedia_transport *tp)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
    PJ_ASSERT_RETURN(tp, PJ_EINVAL);
 ...
 */
static pj_status_t pj_zrtp_transport_simulate_lost(pjmedia_transport *tp, pjmedia_dir dir, unsigned pct_lost)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
 ...
 */
static pj_status_t pj_zrtp_transport_destroy(pjmedia_transport *tp)
{
    struct tp_zrtp *zrtp = (struct tp_zrtp*)tp;
