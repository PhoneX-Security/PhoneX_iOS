...
const char* zrtpStatesName[] = {
        ...
        "MaxStates"           /*!< Gives total number of protocol states */
};
...
#define PH4_DLSYM(cb, handle, fction) do {                              \
    ...
    if (dlerr!=NULL){                                                   \
        PJ_LOG(1, (TAG, "Cannot resolve [%s.%s]. Error: %s", #cb, #fction, dlerr));  \
...
@implementation PEXPjZrtp {

}
+ (PEXPjZrtp *)instance {
    static PEXPjZrtp *_instance = nil;
...
-(int) loadlib: (NSString *) glueLibPath zrtpLibPath: (NSString *) zrtpLibPath{
#ifdef ZRTP_DYNAMIC_LINKING
    ...
    if (zrtpCppHandle==NULL){
        PJ_LOG(1, (TAG, "Cannot open library [%s]. Error: %s", zrtp_lib_path, dlerror()));
        return 0x1;
    }

    zrtpGlueHandle = dlopen(glue_lib_path, RTLD_LAZY);
    if (zrtpGlueHandle==NULL){
        PJ_LOG(1, (TAG, "Cannot open library [%s]. Error: %s", glue_lib_path, dlerror()));
        return 0x2;
    }
#endif
...
+(PEXPjZrtpInfo *) newInfoFromData: (zrtp_cb_user_data*) data{
    return [PEXPjZrtpInfo infoWithCall_id:data->call_id zrtp_tp:data->zrtp_tp];
}

-(void) zrtpShowSas: (void*) data sas:(char*) sas verified:(int) verified{
    ...
    NSString * sasStr = [NSString stringWithCString:sas encoding:NSASCIIStringEncoding];
    [self.delegate zrtpShowSas:[PEXPjZrtp newInfoFromData:zrtp_data] sas:sasStr verified:verified];
}

-(void) zrtpSecureOn: (void*) data cipher: (char*) cipher{
    DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    pj_strdup2_with_null(zrtp_data->pool, &zrtp_data->cipher, cipher);

    NSString * cipherStr = [NSString stringWithCString:cipher encoding:NSASCIIStringEncoding];
    [self.delegate zrtpSecureOn:[PEXPjZrtp newInfoFromData:zrtp_data] cipher:cipherStr];
}

-(void) zrtpSecureOff:(void*) data{
    DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    [self.delegate zrtpSecureOff:[PEXPjZrtp newInfoFromData:zrtp_data]];
}

-(void) confirmGoClear:(void*) data{
    DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    [self.delegate confirmGoClear: [PEXPjZrtp newInfoFromData:zrtp_data]];
}

-(void) showMessage:(void*) data sev: (int32_t) sev subCode: (int32_t) subCode{
    DDLogVerbose(@"%@: %@, sev=%d, subCode=%d", THIS_FILE, THIS_METHOD, sev, subCode);
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    [self.delegate showMessage:[PEXPjZrtp newInfoFromData:zrtp_data] sev:sev subCode:subCode];
//
//    // propagate to UI
//    on_zrtp_update_state_wrapper(zrtp_data->call_id, 1, 0, sev, subCode);
//    switch (sev)
//    {
//        case zrtp_Info:
//            PJ_LOG(3,(TAG, "ZRTP info message: %s", InfoCodes[subCode]));
//            if(subCode == zrtp_InfoSecureStateOn
//                    || subCode == zrtp_InfoSecureStateOff){
//                if(zrtp_data != NULL){
//                    on_zrtp_update_transport_wrapper(zrtp_data->call_id);
//                }else{
//                    PJ_LOG(1, (TAG, "Got a message without associated call_id"));
//                }
//            }
//            break;
//
//        case zrtp_Warning:
//            PJ_LOG(3,(TAG, "ZRTP warning message: %s", WarningCodes[subCode]));
//            break;
//
//        case zrtp_Severe:
//            PJ_LOG(3,(TAG, "ZRTP severe message: %s", SevereCodes[subCode]));
//            break;
//
//        case zrtp_ZrtpError:
//            PJ_LOG(1,(TAG, "ZRTP Error: subcode: %" PRId32, subCode));
//            break;
//    }
}

-(void) zrtpNegotiationFailed:(void*) data severity: (int32_t) severity subcode: (int32_t) subCode{
    DDLogVerbose(@"%@: %@; %" PRId32 ", subcode: %"PRId32, THIS_FILE, THIS_METHOD, severity, subCode);
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    [self.delegate zrtpNegotiationFailed:[PEXPjZrtp newInfoFromData:zrtp_data] severity:severity subcode:subCode];
}

-(void) zrtpNotSuppOther:(void*) data{
    DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    [self.delegate zrtpNotSuppOther:[PEXPjZrtp newInfoFromData:zrtp_data]];
}

-(void) zrtpAskEnrollment:(void*) data info: (int32_t) info{
    DDLogVerbose(@"ZRTP - Ask PBX enrollment");
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    [self.delegate zrtpAskEnrollment:[PEXPjZrtp newInfoFromData:zrtp_data] info:info];
}

-(void) zrtpInformEnrollment:(void*) data info: (int32_t) info{
    DDLogVerbose(@"ZRTP - Inform PBX enrollement");
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    [self.delegate zrtpInformEnrollment:[PEXPjZrtp newInfoFromData:zrtp_data] info:info];
}

-(void) signSAS:(void*) data sas: (uint8_t*) sas{
    DDLogVerbose(@"ZRTP - sign SAS");
}

-(int32_t) checkSASSignature:(void*) data sas: (uint8_t*) sas{
    DDLogVerbose(@"ZRTP - check SAS signature");
    return 0;
}

-(int32_t) checkZrtpHashMatch:(void*) data matchResult: (int32_t) matchResult{
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    DDLogVerbose(@"ZRTP - check ZRTP hash match [%d]", matchResult);
    zrtp_data->zrtp_hash_match = matchResult;
    [self.delegate checkZrtpHashMatch:[PEXPjZrtp newInfoFromData:zrtp_data] matchResult:matchResult];
    return 0;
}

- (void)transportDestroy:(void *)data {
    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) data;
    DDLogVerbose(@"ZRTP transport destroyed %p", data);
    if (data == NULL){
        return;
    }
                                          ...
                                          flags: (unsigned) flags
{
    ...
    if (media_idx < call->med_prov_cnt) {
        ...
        if (call_med->tp && call_med->type != PJMEDIA_TYPE_AUDIO) {
            PJ_LOG(2, (TAG, "ZRTP transport not yet supported for : %d", call_med->type));
            return base_tp;
        }
    }

    if (![self isLinkingValid]){
        PJ_LOG(1, (TAG, "ZRTP library is not loaded. "));
        return base_tp;
    }
    ...
    if(status == PJ_SUCCESS && pool != NULL){
        PJ_LOG(4,(TAG, "ZRTP transport created"));

        // Build callback data pointer
        ...
        */
        NSString * zrtpFile = [self.configuration getZrtpFile];
        PH4_INVOKE(zrtplib_transport_cb, pjmedia_transport_zrtp_initialize, (zrtp_tp, [zrtpFile cStringUsingEncoding:NSASCIIStringEncoding], PJ_TRUE));
        return zrtp_tp;
    } else {
        PJ_LOG(1, (TAG, "ZRTP transport problem : %d", status));
        return base_tp;
    }
}
...
-(int) jzrtp_getCallId:(long) zrtp_data_p{
//    zrtp_cb_user_data* zrtp_data = (zrtp_cb_user_data*) zrtp_data_p;
//    return zrtp_data->call_id;
    return 0;
}

-(NSString *)getInfo: (pjmedia_transport*) tp{
    NSString * result = nil;
    if (![self isLinkingValid]){
        PJ_LOG(1, (TAG, "ZRTP library is not loaded. "));
        return result;
    ...
    if (state) {
       result = [NSString stringWithFormat:@"ZRTP - %s\n%.*s\n%.*s\nSt: %d", "OK",
                       (int)zrtp_cb_data->sas.slen, zrtp_cb_data->sas.ptr,
                       (int)zrtp_cb_data->cipher.slen, zrtp_cb_data->cipher.ptr,
                       state];
    }

    DDLogVerbose(@"ZRTP getInfo: [%@], state=%d", result, state);
    return result;
}

-(struct jzrtp_allContext)getContext: (pjsua_call_id) call_id {
    pjsua_call *call;
    pj_status_t status;
    unsigned i;
    ...
    if (call_id < 0 || call_id > (int)pjsua_var.ua_cfg.max_calls){
        DDLogError(@"Call id is not valid: %d", call_id);
        return result;
    }

    if (![self isLinkingValid]){
        DDLogError(@"ZRTP library is not loaded. ");
        return result;
    }

    if (!pjsua_call_has_media(call_id)) {
        DDLogVerbose(@"No call media found for call %d", call_id);
        return result;
    }
    ...
    for (i = 0; i < call->med_cnt; ++i) {
        ...
        for (unsigned j = 0; j < tp_info.specific_info_cnt; ++j) {
            ...
            if (tp_info.spc_info[j].type == PJMEDIA_TRANSPORT_TYPE_ICE){
                ...
                PJ_LOG(4, (TAG, "PJMEDIA_ICE transport detected; role:[%s] state_name:[%s] comp_cnt:[%u]",
                ...
                if (tp_ice==NULL){
                    PJ_LOG(3, (TAG, "PJMEDIA_ICE transport structure is null"));
                    continue;
                }
                ...
                if (ice_st==NULL){
                    PJ_LOG(3, (TAG, "PJMEDIA_ICE pj_ice_strans defining structure is null"));
                    continue;
                }
                ...
                if (ice_cb==NULL){
                    PJ_LOG(3, (TAG, "PJMEDIA_ICE callback structure is null"));
                }

                // obtain ICE state here
                pj_ice_strans_state ice_state = pj_ice_strans_get_state(ice_st);
                PJ_LOG(4, (TAG, "PJMEDIA_ICE state obtained = %d", ice_state));
                */
            }
...
-(void)sasVerified: (pjsua_call_id) call_id {
    struct jzrtp_allContext ac = [self getContext:call_id];
    ...
    if(ac.zrtpContext != NULL){
        ...
        PJ_LOG(1, (TAG, "sasVerified: No ZRTP context for call %d", call_id));
    }
}
...
-(void)sasRevoked: (pjsua_call_id) call_id {
    ...
    if(ac.zrtpContext != NULL){
        ...
        PJ_LOG(1, (TAG, "sasRevoked: No ZRTP context for call %d", call_id));
    }
}

-(PEXPjZrtpStateInfo *)getInfoFromContext: (struct jzrtp_allContext) ac{
    PEXPjZrtpStateInfo * info = [[PEXPjZrtpStateInfo alloc] init];

    PJ_LOG(4, (TAG, "jzrtp_getInfoFromContext : user data %x", ac.cbUserData));
    if (![self isLinkingValid]){
        PJ_LOG(1, (TAG, "ZRTP library is not loaded. "));
        return info;
    }
    ...
    if(ac.zrtpContext != NULL){
        ...
        if(ac.cbUserData){
            info.sas_verified = ac.cbUserData->sas_verified > 0;
            info.call_id = ac.cbUserData->call_id;
            info.zrtp_hash_match = ac.cbUserData->zrtp_hash_match;
            info.sas = ac.cbUserData->sas.slen == 0 ? nil : [PEXPjUtils copyToString:&ac.cbUserData->sas];
            info.cipher = ac.cbUserData->cipher.slen == 0 ? nil : [PEXPjUtils copyToString:&ac.cbUserData->cipher];
        }
...
-(PEXPjZrtpStateInfo *)getInfoFromCall: (pjsua_call_id) call_id{
    PJSUA_LOCK();
    struct jzrtp_allContext ctxt = [self getContext:call_id];
    PEXPjZrtpStateInfo * info = [self getInfoFromContext:ctxt];
    PJSUA_UNLOCK();
    return info;
}
...
-(pj_status_t) dump_zrtp:(pjsua_call_id) call_id buffer: (char *) buffer maxlen: (unsigned) maxlen indent: (const char *) indent{
            ...
            if (call_med->tp) {
                ...
                if (tp_info.specific_info_cnt > 0) {
                    ...
                    for (j = 0; j < tp_info.specific_info_cnt; ++j) {
                        PJ_LOG(4, (TAG, "Media transport type [%d]; idx=%d ", tp_info.spc_info[j].type, j));
                        ...
                        if (tp_info.spc_info[j].type == PJMEDIA_TRANSPORT_TYPE_ICE){
                            ...
                            ii = (const pjmedia_ice_transport_info*) tp_info.spc_info[j].buffer;
                            PJ_LOG(4, (TAG, " ICE media here comp_cnt:[%u] ", ii->comp_cnt));
...
-(NSString *)zrtp_call_dump:(pjsua_call_id)call_id indent: (const char *) indent{
    const size_t numBytes = 1024*5;
    NSMutableData * buff = [NSMutableData dataWithLength:numBytes];
    pj_status_t status = [self dump_zrtp:call_id buffer:[buff mutableBytes] maxlen:numBytes indent:indent];
    if(status != PJ_SUCCESS){
        pj_strerror(status, [buff mutableBytes], numBytes);
    }

    return [NSString stringWithCString:[buff mutableBytes] encoding:NSUTF8StringEncoding];
}
...
static void zrtpShowSas(void* data, char* sas, int verified){
    [[PEXPjZrtp instance] zrtpShowSas:data sas:sas verified:verified];
}

static void zrtpSecureOn(void* data, char* cipher){
    [[PEXPjZrtp instance] zrtpSecureOn:data cipher:cipher];
}

static void zrtpSecureOff(void* data){
    [[PEXPjZrtp instance] zrtpSecureOff:data];
}

static void confirmGoClear(void* data){
    [[PEXPjZrtp instance] confirmGoClear:data];
}

static void showMessage(void* data, int32_t sev, int32_t subCode){
    [[PEXPjZrtp instance] showMessage:data sev:sev subCode:subCode];
}

static void zrtpNegotiationFailed(void* data, int32_t severity, int32_t subCode){
    [[PEXPjZrtp instance] zrtpNegotiationFailed:data severity:severity subcode:subCode];
}

static void zrtpNotSuppOther(void* data){
    [[PEXPjZrtp instance] zrtpNotSuppOther:data];
}

static void zrtpAskEnrollment(void* data, int32_t info){
    [[PEXPjZrtp instance] zrtpAskEnrollment:data info:info];
}

static void zrtpInformEnrollment(void* data, int32_t info){
    [[PEXPjZrtp instance] zrtpInformEnrollment:data info:info];
}

static void signSAS(void* data, uint8_t* sas){
    [[PEXPjZrtp instance] signSAS:data sas:sas];
}

static int32_t checkSASSignature(void* data, uint8_t* sas){
    return [[PEXPjZrtp instance] checkSASSignature:data sas:sas];
}

static int32_t checkZrtpHashMatch(void* data, int32_t matchResult){
    return [[PEXPjZrtp instance] checkZrtpHashMatch:data matchResult:matchResult];
}

static void transportDestroy(void* data){
    return [[PEXPjZrtp instance] transportDestroy:data];
}
