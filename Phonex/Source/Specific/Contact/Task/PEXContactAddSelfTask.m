...
@interface PEXContactAddSelfTask () { }
...
@implementation PEXContactAddSelfTask
...
- (void)cancel {
    ...
    DDLogVerbose(@"Cancel called at PEXContactAddSelfTask.");
...
-(void) checkIfAlreadyAdded {
    PEXDbContentProvider * cr = [PEXDbAppContentProvider instance];
    PEXDbContact * desiredContact = [PEXDbContact newProfileFromDbSip:cr sip:self.contactAddress
                                                           projection:[PEXDbContact getLightProjection]];

    // Already added ?
    if (desiredContact != nil && [self.contactAddress isEqualToString:desiredContact.sip]){
