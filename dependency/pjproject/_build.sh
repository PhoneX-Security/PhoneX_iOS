...
cp "${CONFIG_SITE}" "${DIR}/pjlib/include/pj/config_site.h" && \
cp "${CONFIG_SITE}" "${UDIR}/pjlib/include/pj/config_site.h" 
if [ $? -ne 0 ]; then
...
export COMMON_CFLAGS="${OPTIMIZE_FLAGS} -Wno-unused-label -I"${OPENSSL}"/include"
export COMMON_LDFLAGS="${OPTIMIZE_FLAGS} -L"${OPENSSL}"/lib -lcrypto -lssl"
...
function buildArchitecture() {
    export ARCH="-arch "${1}""
	"${DIR}"/configure-iphone --enable-ssl --with-ssl="${OPENSSL}"
	if [ $? -ne 0 ]; then
	...
	for PJ_COMPONENT in ${PJ_COMPONENTS[*]}
	do
		FOLDER_PATH=""${DIR}"/"${PJ_COMPONENT}"/lib"
		storeArchitecture "${1}" "${FOLDER_PATH}"
	done

	storeArchitecture "${1}" ""${THIRD_PARTY_FOLDER}"/lib"
}
...
function lipoBuild() {
	...
	lipoLib ""${THIRD_PARTY_FOLDER}"/lib"
}
